...
实现同一个问题的代码组织方式有无穷多种，而且各自都有不同取舍。本节的实现更偏向一种传统的面向对象风格：在 Rust 里当然可以这样写，但它并没有利用 Rust 的某些优势。稍后我们会展示另一种仍然使用面向对象设计模式、但结构上对有面向对象经验的程序员来说可能不那么熟悉的做法。通过比较这两种方案，我们就能体会到：以不同于其他语言的方式来设计 Rust 代码，会带来怎样的权衡。
