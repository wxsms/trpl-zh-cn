...
之所以能够在 `add` 调用中使用 `&s2`，是因为编译器可以把 `&String` 参数强制转换成 `&str`。当调用 `add` 方法时，Rust 会使用一种叫做 **deref 强制转换**（*deref coercion*）的机制，这里会把 `&s2` 转换成 `&s2[..]`。第十五章会更深入地讨论 deref 强制转换。因为 `add` 不会获取 `s` 参数的所有权，所以在这个操作之后，`s2` 仍然是一个有效的 `String`。
